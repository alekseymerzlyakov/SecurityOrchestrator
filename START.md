...
*   **AI Only** — только AI анализ (глубокий аудит всего кода).
*   **Hybrid (Рекомендуется)** — тандем инструментов и ИИ:
    1.  **Сбор находок**: Сначала работают базовые сканеры (Semgrep, Gitleaks и др.).
    2.  **Передача контекста**: Все найденные уязвимости передаются ИИ как "подсказки" (hints).
    3.  **Smart Hybrid**: ИИ фокусируется на "горячих точках" (файлы с находками + их соседи), не тратя бюджет на заведомо чистые файлы.
    4.  **Верификация**: ИИ проверяет находки инструментов на False Positive и ищет сложные цепочки атак.
